----------------------------------------------------------------------------------------
--
-- File name:   cs_blackout_lock.sql
--
-- Purpose:     Blackout Lock - for IOD/DBPERF Jobs
--
...
--
EXEC &&cs_tools_schema..IOD_LOG.configure(logName => 'cs_blackout_lock.sql', logToOutput => 'Y', logToAlertLog => 'Y'); 
EXEC &&cs_tools_schema..IOD_LOG.info(msg => 'Lock Blackout');
--
DECLARE
  l_count INTEGER;
...
BEGIN    
  ...
  ELSIF SYSDATE BETWEEN l_rec.begin_time AND l_rec.end_time THEN
    ...
    UPDATE &&cs_tools_schema..blackout SET locked = 'Y' WHERE id = 1;
    COMMIT;
