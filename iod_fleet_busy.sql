...
SELECT /*+ MATERIALIZE NO_MERGE */
 ...
 WHERE version = '&&cs_version.'
   AND (
             ROUND(100 * load_avg / num_cpu_threads) >= 50
          OR ROUND(100 * load_p90 / num_cpu_threads) >= 50 
          OR ROUND(100 * load_p95 / num_cpu_threads) >= 65
          OR ROUND(100 * load_p99 / num_cpu_threads) >= 95 
          OR ROUND(aas_on_cpu_avg) >= num_cpu_threads
       --    OR aas_on_cpu_p90 >= num_cpu_threads
       --    OR aas_on_cpu_p95 >= num_cpu_threads
       --    OR aas_on_cpu_p99 >= num_cpu_threads
          OR fs_u02_util_perc >= 80
          OR TO_DATE(fs_u02_at_80p) < SYSDATE + 31
          OR TO_DATE(fs_u02_at_90p) < SYSDATE + 31
          OR TO_DATE(fs_u02_at_95p) < SYSDATE + 31
   )
),
ordered AS (
SELECT realm_type, 
       ...
       host_name,
       CASE
         WHEN fs_u02_util_perc_now > 88 THEN 1
         WHEN disk_config = 'striped' AND host_shape LIKE 'x7.enclave2%' AND fs_u02_util_perc_now > 80 AND TO_DATE(fs_u02_at_90p) - SYSDATE BETWEEN 0 AND 30 THEN 1
         WHEN fs_u02_util_perc_now > 80 AND TO_DATE(fs_u02_at_90p) - SYSDATE BETWEEN 0 AND 45 THEN 2
         WHEN disk_config = 'striped' AND host_shape LIKE 'x7.enclave2%' AND fs_u02_util_perc_now > 70 AND TO_DATE(fs_u02_at_90p) - SYSDATE BETWEEN 0 AND 90 THEN 2
         WHEN fs_u02_util_perc_now > 70 AND TO_DATE(fs_u02_at_90p) - SYSDATE BETWEEN 0 AND 120 THEN 3
       END AS priority
  FROM all_db_servers
 ORDER BY
...
SELECT 'Rgn,DB Name,CPU Cores per Server,CPU Threads per Server,Thread Util % Avg,Thread Util % P90,Thread Util % P95,Thread Util % P99,Disk TB per Server,Used TB per Server,Used TB last Month,Current Space Util %,Last Month Space Util %,Disk at 80%,Disk at 90%,Disk at 95%,Priority,Disk Config,Host Shape,PDBs,Primary Host Name' AS line
  FROM DUAL
 UNION ALL
SELECT 
       -- realm_type||','||
