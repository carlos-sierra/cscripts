...
---------------------------------------------------------------------------------------
--
-- exit graciously if executed on standby or bystander
...
BEGIN
  ...
  FOR i IN
  (WITH s AS
  (SELECT sql_id, address, hash_value, sql_text, is_obsolete, plan_hash_value, con_id FROM v$sql WHERE rownum<165000
  )
SELECT sql_id,
  address,
  hash_value,
...
ORDER BY sql_id,
  ...
  LOOP
    IF SYSDATE + (1/1440) >= l_timeout THEN
      EXIT;
    END IF; -- exits 1m before timeout of 1h (for ease of 1h OEM scheduling)
    ...
    BEGIN
      ...
      IF i.sql_id <> l_prior_sql_id THEN
        l_sqls    := l_sqls + 1;
      END IF;
