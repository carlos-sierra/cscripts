-- IOD_REPEATING_SPACE_MAINTENANCE (daily at 4PM UTC) KIEV
-- Purge Recyclebin and performs Online Table Redefinition
--
DEF report_only = 'N';
...
DEF pdb_name = '';
--
-- Exclude one specific table: INSERT INTO c##iod.exceptions_black_list (iod_api, pdb_name, owner, table_name, reference) VALUES ('TABLE_REDEFINITION', 'FLAMINGO_OPS', 'KIEVUSER', 'HISTORICALASSIGNMENT', 'IOD-12345');
-- Exclude one specific owner: INSERT INTO c##iod.exceptions_black_list (iod_api, pdb_name, owner, reference) VALUES ('TABLE_REDEFINITION', 'FLAMINGO_OPS', 'KIEVUSER', 'IOD-12345');
-- Exclude one specific pdb: INSERT INTO c##iod.exceptions_black_list (iod_api, pdb_name, reference) VALUES ('TABLE_REDEFINITION', 'FLAMINGO_OPS', 'IOD-12345');
-- Exclude one table for all pdbs: INSERT INTO c##iod.exceptions_black_list (iod_api, table_name, reference) VALUES ('TABLE_REDEFINITION', 'HISTORICALASSIGNMENT', 'IOD-12345');
--
...
--
-- exit graciously if executed on telemetry or casper
WHENEVER SQLERROR EXIT SUCCESS;
DECLARE
  l_host_name VARCHAR2(64);
BEGIN
  SELECT host_name INTO l_host_name FROM v$instance;
  IF LOWER(l_host_name) LIKE CHR(37)||'telemetry'||CHR(37) OR LOWER(l_host_name) LIKE CHR(37)||'casper'||CHR(37) THEN
    raise_application_error(-20000, 'Cannot execute on host: '||l_host_name);
  END IF;
END;
/
WHENEVER SQLERROR CONTINUE;
