...
SPO small_tables_queried_&&current_time..txt;
...
SELECT q.con_id, 
       ...
       t.table_name, 
       COUNT(*) plans,
       SUM(q.executions) executions,
       ROUND(SUM(q.elapsed_time) / SUM(q.executions)) us_per_exec,
       ROUND(SUM(q.buffer_gets) / SUM(q.executions)) bg_per_exec,
       MAX(t.num_rows) num_rows, 
       TO_CHAR(MAX(t.last_analyzed), 'YYYY-MM-DD"T"HH24:MI:SS') last_analyzed, 
       MAX(q.sql_text_100) sql_text_100
