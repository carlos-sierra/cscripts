--IOD_IMMEDIATE_COMPUTE_TENANT 
--Executes sqlperf.sql for a9u7pptds12g5 in all COMPUTE pdbs
DEF 1 = 'a9u7pptds12g5';
--
----------------------------------------------------------------------------------------
--
...
SELECT '/tmp/sqlperf_'||LOWER(name)||'_'||LOWER(REPLACE(SUBSTR(host_name, 1 + INSTR(host_name, '.', 1, 2), 30), '.', '_'))||'_'||LOWER(SYS_CONTEXT('USERENV','CON_NAME'))||'_&&sql_id._'||TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') output_file_name FROM v$database, v$instance;
