...
--
PRO
PRO SQL MONITOR REPORTS (dba_hist_reports) top &&cs_sqlmon_top. last &&cs_awr_days. days
PRO ~~~~~~~~~~~~~~~~~~~
WITH
...
FROM   cdb_hist_reports r, 
       XMLTABLE('//sql' passing xmltype(r.report_summary) COLUMNS sql_id 
       VARCHAR2(13) 
       path '@sql_id', sql_exec_start VARCHAR2(19) path '@sql_exec_start', 
       sql_exec_id 
       NUMBER path '@sql_exec_id' ) xt 
WHERE  r.component_name = 'sqlmonitor' 
       AND report_name = 'main' 
       AND r.key1 = '&&cs_sql_id.' 
       AND r.period_start_time > SYSDATE - &&cs_awr_days.
