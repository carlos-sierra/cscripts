...
SELECT h.plan_hash_value,
       SUM(h.elapsed_time_delta)/NULLIF(SUM(h.executions_delta), 0)/1e3 avg_et_ms,
       SUM(h.cpu_time_delta)/NULLIF(SUM(h.executions_delta), 0)/1e3 avg_cpu_ms,
       SUM(h.iowait_delta)/NULLIF(SUM(h.executions_delta), 0)/1e3 avg_io_ms,
       SUM(h.apwait_delta)/NULLIF(SUM(h.executions_delta), 0)/1e3 avg_appl_ms,
       SUM(h.ccwait_delta)/NULLIF(SUM(h.executions_delta), 0)/1e3 avg_conc_ms,
       SUM(h.buffer_gets_delta)/NULLIF(SUM(h.executions_delta), 0) avg_bg,
       SUM(h.disk_reads_delta)/NULLIF(SUM(h.executions_delta), 0) avg_disk,
       SUM(h.physical_read_bytes_delta)/NULLIF(SUM(h.executions_delta), 0) avg_read_bytes,
       SUM(h.physical_write_bytes_delta)/NULLIF(SUM(h.executions_delta), 0) avg_write_bytes,
       SUM(h.rows_processed_delta)/NULLIF(SUM(h.executions_delta), 0) avg_row,
       ...
       SUM(h.buffer_gets_delta) total_buffer_gets,
       SUM(h.disk_reads_delta) total_disk_reads,
       SUM(h.physical_read_bytes_delta) total_read_bytes,
       SUM(h.physical_write_bytes_delta) total_write_bytes,
       SUM(h.rows_processed_delta) total_rows_processed,
