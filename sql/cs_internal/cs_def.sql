...
DEF cs_me_last = '10';
DEF cs_me_days = '30';
DEF cs_aas_on_cpu_per_sql = '1';
DEF cs_cpu_ms_per_row = '0.500';
DEF cs_buffer_gets_per_row = '20';
...
--
SELECT C##IOD.IOD_META_AUX.get_region('&&cs_host_name.') AS cs_region FROM DUAL
/
--
SELECT C##IOD.IOD_META_AUX.get_realm('&&cs_region.') AS cs_realm, C##IOD.IOD_META_AUX.get_region_acronym('&&cs_region.') AS cs_rgn FROM DUAL
/
--
SELECT C##IOD.IOD_META_AUX.get_locale(value) AS cs_locale FROM v$parameter WHERE name = 'db_domain'
/
--
COL zapper_actions_script NEW_V zapper_actions_script NOPRI;
SELECT CASE WHEN COUNT(*) > 0 THEN 'cs_zapper_actions.sql' ELSE 'cs_null.sql' END AS zapper_actions_script
  FROM dba_tables
 WHERE owner = UPPER('&&cs_stgtab_owner.')
   AND table_name = UPPER('sql_plan_baseline_hist')
/
--
