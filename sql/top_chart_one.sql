...
PRO Range of snaps below are to define lower and upper bounds to compute TOP.
PRO
PRO Enter range of snaps to evaluate TOP. Default is last &&default_window_hours. hours.
PRO
...
PRO
PRO TOP is computed for selected metric within range of snaphots.
PRO
PRO Computed Metric
PRO ~~~~~~~~~~~~~~~
...
DEF report_abstract_6 = "<br>SQL Text piece: &&sql_text_piece.";
DEF report_abstract_7 = "<br>SQL_ID: &&sql_id.";
DEF chart_title = "Top &&top_what. according to &&metric_display. between &&begin_interval_time.(&&snap_id_min.) and &&end_interval_time.(&&snap_id_max.) UTC";
...
/
DEF vaxis_baseline = "";;
DEF report_title = "Top &&top_what. according to &&metric_display.<br>between &&begin_interval_time.(&&snap_id_min.) and &&end_interval_time.(&&snap_id_max.) UTC";
...
SELECT /*+ MATERIALIZE NO_MERGE */
       ...
       CASE WHEN rs.rank <= &&top_n. THEN (CASE WHEN '&&sql_id.' IS NULL THEN -666 ELSE h.plan_hash_value END) END plan_hash_value,
 ...
 GROUP BY
       ...
       CASE WHEN rs.rank <= &&top_n. THEN (CASE WHEN '&&sql_id.' IS NULL THEN -666 ELSE h.plan_hash_value END) END,
...
SELECT /*+ MATERIALIZE NO_MERGE FULL(rs) */
       ...
       (CASE '&&con_name.' WHEN 'CDB$ROOT' THEN ' '||c.name END)||'''' line
  FROM ranked_sql rs,
       v$containers c
 WHERE rs.rank <= &&top_n.
