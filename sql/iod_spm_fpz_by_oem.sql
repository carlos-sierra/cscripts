DEF pdb_name_1 = 'COMPUTE';
DEF pdb_name_2 = 'COMPUTE_WF';
...
SELECT '/tmp/iod_spm_fpz_&&pdb_name_1._'||TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') report_file FROM DUAL;
SPO &&report_file..txt;
EXEC c##iod.iod_spm.fpz(p_report_only => 'N', p_pdb_name => UPPER(TRIM('&&pdb_name_1.')));
SPO OFF;
HOS zip -mj &&zip_file..zip &&report_file..txt
SELECT '/tmp/iod_spm_fpz_&&pdb_name_2._'||TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') report_file FROM DUAL;
SPO &&report_file..txt;
EXEC c##iod.iod_spm.fpz(p_report_only => 'N', p_pdb_name => UPPER(TRIM('&&pdb_name_2.')));
SPO OFF;
HOS zip -mj &&zip_file..zip &&report_file..txt
