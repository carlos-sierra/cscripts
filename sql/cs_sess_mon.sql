...
--
-- Purpose:     Session Monitor Details
--
...
DEF only_killed = 'N';
PRO
PRO Monitored Sessions (c##iod.sess_mon_session)
PRO ~~~~~~~~~~~~~~~~~~
--
WITH
sqf_session AS (
...
SELECT /*+ MATERIALIE NO_MERGE */
       ...
       t.api,
       COUNT(*) AS cnt
  FROM c##iod.sess_mon_transaction t
...
SELECT /*+ MATERIALIE NO_MERGE */
       ...
       w.api,
       COUNT(*) AS cnt
  FROM c##iod.sess_mon_wait_chains w
...
DEF only_killed = 'Y';
PRO
PRO Killed Sessions (c##iod.sess_mon_session)
PRO ~~~~~~~~~~~~~~~
/
--
PRO
...
SELECT /*+ MATERIALIZE NO_MERGE */
       ...
       CASE
         WHEN (SELECT COUNT(*)
                 FROM c##iod.sess_mon_wait_chains b -- blockee
...
/
PRO
@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_wait_chains WHERE ''&&cs_con_name.'' IN (''CDB$ROOT'', pdb_name) AND snap_time = TO_DATE(''&&snap_time.'')";
PRO
PRO Sessions (v$session)
PRO ~~~~~~~~
@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_session WHERE ''&&cs_con_name.'' IN (''CDB$ROOT'', pdb_name) AND snap_time = TO_DATE(''&&snap_time.'') ORDER BY final_blocking_session NULLS FIRST, sid";
PRO
PRO Locks (v$lock)
PRO ~~~~~
@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_lock WHERE ''&&cs_con_name.'' IN (''CDB$ROOT'', pdb_name) AND snap_time = TO_DATE(''&&snap_time.'') ORDER BY sid, type, id1";
PRO
PRO Transactions (v$transaction)
PRO ~~~~~~~~~~~~
@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_transaction WHERE ''&&cs_con_name.'' IN (''CDB$ROOT'', pdb_name) AND snap_time = TO_DATE(''&&snap_time.'') ORDER BY addr";
--PRO
--PRO Library Cache Pins (x$kglpn)
--PRO ~~~~~~~~~~~~~~~~~~
--@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_lc_pin WHERE snap_time = TO_DATE(''&&snap_time.'')";
--PRO
--PRO Library Cache Locks (x$kgllk)
--PRO ~~~~~~~~~~~~~~~~~~~
--@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_lc_lock WHERE snap_time = TO_DATE(''&&snap_time.'')";
PRO
PRO Objects (dba_objects)
PRO ~~~~~~~
@@cs_internal/cs_pr_internal.sql "SELECT * FROM c##iod.sess_mon_objects WHERE ''&&cs_con_name.'' IN (''CDB$ROOT'', pdb_name) AND snap_time = TO_DATE(''&&snap_time.'') ORDER BY object_id";
--
PRO
PRO SQL> @&&cs_script_name..sql "&&snap_time."
--
