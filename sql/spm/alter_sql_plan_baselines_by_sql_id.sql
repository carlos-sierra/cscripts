----------------------------------------------------------------------------------------
--
-- File name:   alter_sql_plan_baselines_by_sql_id.sql
--
-- Purpose:     Modifies SQL Plan Baseline for a given SQL_ID
--
...
--              SQL> @alter_sql_plan_baselines_by_sql_id.sql
-- 
--              pass string such as: 
--                d0r59da3g1mbj
--
...
SPO alter_sql_plan_baselines_by_sql_id_&&sql_id._&&x_container._&&current_time..txt;
...
ACC attribute_name PROMPT 'Enter Attribute Name (ENABLED, FIXED, AUTOPURGE or DESCRIPTION): ';
...
BEGIN
  ...
  LOOP
    ...
    l_plans := DBMS_SPM.ALTER_SQL_PLAN_BASELINE(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => '&&attribute_name.', attribute_value => '&&attribute_value.');
  END LOOP;
END;
/
