...
PRO rows_min     : Rows Processed per Minute
PRO rows_exec    : Rows Processed per Exec
PRO reads_min    : Buffer Gets and Disk Reads per Minute
...
PRO Rows         : time_per_row, rows_min, rows_exec, reads_per_row
PRO Gets         : reads_min, reads_exec, reads_per_row
PRO *            : All
PRO
...
UNION ALL SELECT '@@cs_internal/cs_sqlstat_chart_internal.sql "rows_min"' FROM DUAL WHERE '&&metric_group.' IN ('rows_min', 'Rows', '*')
UNION ALL SELECT '@@cs_internal/cs_sqlstat_chart_internal.sql "rows_exec"' FROM DUAL WHERE '&&metric_group.' IN ('rows_exec', 'Rows', 'Main', '*')
UNION ALL SELECT '@@cs_internal/cs_sqlstat_chart_internal.sql "reads_min"' FROM DUAL WHERE '&&metric_group.' IN ('reads_min', 'Gets', '*')
