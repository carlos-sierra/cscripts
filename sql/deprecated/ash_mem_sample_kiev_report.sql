...
SELECT TO_CHAR(MIN(sample_time),'YYYY-MM-DD"T"HH24:MI:SS') time_from_default, TO_CHAR(MAX(sample_time),'YYYY-MM-DD"T"HH24:MI:SS') time_to_default FROM v$active_session_history
/
PRO
...
/
PRO
PRO 3. KIEV Transaction: [{CBSGU}|C|B|S|G|U|CB|SG] (C=CommitTx B=BeginTx S=Scan G=GC U=Unknown)
COL kiev_tx NEW_V kiev_tx NOPRI;
SELECT NVL('&3.', 'CBSGU') kiev_tx FROM DUAL
/
...
SPO ash_mem_sample_kiev_&&current_time..txt;
PRO
PRO SQL>@ash_mem_sample_kiev_report.sql
PRO
PRO ash_mem_sample_kiev_&&current_time..txt
PRO
...
SELECT /*+ MATERIALIZE NO_MERGE */
 ...
 WHERE CAST(h.sample_time AS DATE) BETWEEN TO_DATE('&&sample_time_from.', 'YYYY-MM-DD"T"HH24:MI:SS') AND TO_DATE('&&sample_time_to.', 'YYYY-MM-DD"T"HH24:MI:SS')
...
SELECT TO_CHAR(CAST(h.sample_time AS DATE), 'YYYY-MM-DD"T"HH24:MI:SS') sample_date_time,
       ...
       MAX(CASE WHEN h.row_number = 1 AND h.sql_id IS NOT NULL THEN (SELECT q.sql_text_100 FROM my_tx_sql q WHERE q.sql_id = h.sql_id AND ROWNUM = 1) END) sql_text_100_only
...
SELECT TO_CHAR(CAST(h.sample_time AS DATE), 'YYYY-MM-DD"T"HH24:MI:SS') sample_date_time,
       ...
       (SELECT q.sql_text_100 FROM my_tx_sql q WHERE q.sql_id = h.sql_id AND ROWNUM = 1) sql_text_100_only
  FROM v$active_session_history h
 WHERE CAST(h.sample_time AS DATE) BETWEEN TO_DATE('&&sample_time_from.', 'YYYY-MM-DD"T"HH24:MI:SS') AND TO_DATE('&&sample_time_to.', 'YYYY-MM-DD"T"HH24:MI:SS')
   AND h.sql_id IN (SELECT sql_id FROM my_tx_sql)
 GROUP BY
