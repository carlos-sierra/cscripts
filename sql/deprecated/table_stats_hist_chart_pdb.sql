...
DEF report_foot_note = "Based on CBO statistics history on sys.wri$_optstat_tab_history";
PRO
SPO table_stats_hist_&&x_container._&&table_owner._&&table_name._&&current_time..html;
PRO <html>
...
WITH
my_query AS (
SELECT h.analyzetime last_analyzed,
       h.rowcnt num_rows
  FROM sys.wri$_optstat_tab_history h,
       dba_objects o
 WHERE h.analyzetime IS NOT NULL
