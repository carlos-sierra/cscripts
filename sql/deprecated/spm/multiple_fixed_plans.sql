...
SELECT con_id,
       ...
       TO_CHAR(MIN(created), 'YYYY-MM-DD"T"HH24:MI:SS') min_created,
 ...
 GROUP BY
       con_id,
       signature
HAVING SUM(CASE WHEN enabled = 'YES' AND accepted = 'YES' AND reproduced = 'YES' THEN 1 ELSE 0 END) > 1
   AND SUM(CASE WHEN enabled = 'YES' AND accepted = 'YES' AND reproduced = 'YES' AND fixed = 'YES' THEN 1 ELSE 0 END) > 1
 ORDER BY
       con_id,
       signature
/
