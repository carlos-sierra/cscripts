----------------------------------------------------------------------------------------
--
-- File name:   top_sql_report_one.sql
--
...
COL distinct_plans FOR 99999 HEA 'PLANS';
COL min_plan_hash_value FOR 9999999999 HEA 'MIN PHV';
COL max_plan_hash_value FOR 9999999999 HEA 'MAX PHV';
...
PRO
PRO SQL> @top_sql_report_one.sql "&&display_awr_days." "&&snap_id_from." "&&snap_id_to." "&&computed_metric." "&&kiev_tx." "&&kiev_bucket."
PRO
...
SELECT /*+ MATERIALIZE NO_MERGE */
       h.con_id,
       h.sql_id,
       MAX(h.distinct_plans) distinct_plans,
...
SELECT /*+ MATERIALIZE NO_MERGE */
       ...
       h.disk_reads_exec,
       h.disk_reads_perc,
       ROW_NUMBER() OVER (ORDER BY h.&&computed_metric. DESC NULLS LAST, h.db_time_aas DESC NULLS LAST, h.executions_sec DESC NULLS LAST, h.rows_processed_sec DESC NULLS LAST, h.con_id, h.sql_id) rank
...
SELECT t.rank,
       ...
       --
       t.distinct_plans,
       t.min_plan_hash_value,
       t.max_plan_hash_value,
...
PRO
PRO Top SQL by "&&metric_display." - SQL Latency, Average Active Sessions and Database Time
PRO ~~~~~~~~~~
--
...
PRO
PRO Top SQL by "&&metric_display." - Database User Calls (Load)
PRO ~~~~~~~~~~
--
COL db_time_exec NOPRI;
COL cpu_time_exec NOPRI;
...
PRO
PRO Top SQL by "&&metric_display." - Resources
PRO ~~~~~~~~~~
--
COL db_time_exec NOPRI;
COL cpu_time_exec NOPRI;
...
PRO
PRO Top SQL by "&&metric_display." - Cursors
PRO ~~~~~~~~~~
--
COL db_time_exec NOPRI;
COL cpu_time_exec NOPRI;
