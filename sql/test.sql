...
SELECT w.instance_number inst,
       ...
       (SELECT v.sql_id
          FROM (SELECT sql_id,
                       ROW_NUMBER() OVER (ORDER BY h.snap_id DESC, h.sample_id DESC NULLS LAST) row_number
                 ...
                 WHERE h.snap_id <= w.snap_id 
                   ...
                   AND h.sample_id <= MIN(w.sample_id) -- non RAC
                   AND h.sample_time <= MIN(w.sample_time) -- non RAC
...
SELECT inst,
       ...
       blocking_sessions_count,
       CASE WHEN sql_id IS NULL THEN blocking_session||','||blocking_session_serial# END missing_session_serial,
       sql_id,
       container_id,       
       object_number,
       object_owner_and_name,
       (SELECT SUBSTR(q.sql_text, 1, 100) FROM v$sql q WHERE q.sql_id = b.sql_id AND q.con_id = b.container_id AND ROWNUM = 1) sql_text_100_only
  FROM blocking b
 ORDER BY
       inst,
       snap,
