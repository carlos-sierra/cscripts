...
CLEAR BREAK;
SPO spb_fix_script_dynamic.sql;
...
'x NUMBER; '||CHR(10)||
'BEGIN '||CHR(10)||
'/* SQL_ID:'||sql_id||', PHV:'||plan_hash_value||', SIGNATURE:'||signature||' */ '||CHR(10)||
'x := DBMS_SPM.ALTER_SQL_PLAN_BASELINE(sql_handle=>'''||sql_handle||''', plan_name=>'''||plan_name||''', attribute_name=>''FIXED'', attribute_value=>''YES''); '||CHR(10)||
'END; '||CHR(10)||
'/'
FROM candidates
/
...
SELECT f.con_id,
 ...
 WHERE f.signature = o.signature
   ...
   AND NVL(s.row_number, -666) <> 1
   AND s.version_count > 100
   AND s.invalidations > 1000
 ORDER BY
       f.con_id,
       f.signature,
       f.sql_handle,
...
'BEGIN '||CHR(10)||
'/* SQL_ID:'||sql_id||', PHV:'||plan_hash_value||', SIGNATURE:'||signature||' */ '||CHR(10)||
'x := DBMS_SPM.ALTER_SQL_PLAN_BASELINE(sql_handle=>'''||sql_handle||''', plan_name=>'''||plan_name||''', attribute_name=>''FIXED'', attribute_value=>''NO''); '||CHR(10)||
'END; '||CHR(10)||
'/'
FROM candidates
/
