...
SELECT '/tmp/kiev_slow_'||LOWER(name)||'_'||LOWER(REPLACE(SUBSTR(host_name, 1 + INSTR(host_name, '.', 1, 2), 30), '.', '_'))||'_'||REPLACE(LOWER(SYS_CONTEXT('USERENV','CON_NAME')),'$')||'_'||TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') output_file_name FROM v$database, v$instance;
...
SELECT c.pdb_name,
 ...
 WHERE 1 = 1
   ...
   AND s.elapsed_time / GREATEST(s.executions, 1) / 1e6 > TO_NUMBER('&&et_secs_per_exec.')
   AND c.con_id = s.con_id
 ORDER BY
       s.elapsed_time / GREATEST(s.executions, 1) DESC
/
