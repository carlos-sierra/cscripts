...
/* $Header: iod_space.pkb.sql 2018-06-02T00:56:18 carlos.sierra $ */
/* ------------------------------------------------------------------------------------ */  
...
BEGIN
  ...
  FOR i IN (WITH /*+ iod_space.table_redefinition */
            ...
            rebuild_candidate AS (
            SELECT /*+ MATERIALIZE NO_MERGE ORDERED USE_HASH(t i ts u s l o c) GATHER_PLAN_STATISTICS QB_NAME(rebuild_candidate) */
                   t.con_id, 
                   c.pdb_name,
                   t.owner,
                   t.table_name,
                   ...
                   t.compression,
                   t.compress_for,
                   ROUND(s.bytes / POWER(2,20), 3) table_size_mbs,
                   i.index_count,
                   ROUND(i.sum_bytes / POWER(2,20), 3) all_index_size_mbs,
                   ROUND(i.max_bytes / POWER(2,20), 3) top_index_size_mbs,
                   l.lobs lobs_count,
                   ROUND(l.bytes / POWER(2,20), 3) all_lobs_size_mbs,
  ...
  LOOP
    ...
    THEN
      ...
      output('-- pdb:'||l_table_redefinition_hist_rec.pdb_name||'('||l_table_redefinition_hist_rec.con_id||'). tbl:'||l_table_redefinition_hist_rec.owner||'.'||l_table_redefinition_hist_rec.table_name||'. created:'||TO_CHAR(i.created,gk_date_format)||'. fs:'||l_table_redefinition_hist_rec.full_scan||
             '. lobs:'||l_table_redefinition_hist_rec.lobs_count||'. all_lobs:'||l_table_redefinition_hist_rec.all_lobs_size_mbs_before||'MBs. idx:'||l_table_redefinition_hist_rec.index_count||'. all_idx:'||l_table_redefinition_hist_rec.all_index_size_mbs_before||'MBs. top_idx:'||l_table_redefinition_hist_rec.top_index_size_mbs_before||'MBs.');
      ...
      IF (l_savings_percent > l_min_savings_perc OR l_table_redefinition_hist_rec.top_index_size_mbs_before > l_table_redefinition_hist_rec.table_size_mbs_before) AND l_report_only = 'N' THEN
        ...
        EXCEPTION
          WHEN OTHERS THEN
            -- When getting error below you want to drop first the MV log
            -- ORA-12083: must use DROP MATERIALIZED VIEW to drop "user"."REDEF$_Tnnnnn"
            ...
            output(l_table_redefinition_hist_rec.error_message, p_alert_log => 'Y');
            output('---');
            output('--- remediation example for ORA-12083 (replace "nnnnn"):');
            output('--- ALTER SESSION SET CONTAINER = '||l_table_redefinition_hist_rec.pdb_name||';');
            output('--- DROP MATERIALIZED VIEW LOG ON '||l_table_redefinition_hist_rec.owner||'.'||l_table_redefinition_hist_rec.table_name||';');
            output('--- DROP MATERIALIZED VIEW '||l_table_redefinition_hist_rec.owner||'.REDEF$_Tnnnnn;');
            output('--- DROP TABLE '||l_table_redefinition_hist_rec.owner||'.REDEF$_Tnnnnn;');
            output('---');
            DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
            RAISE;
        END;
        ...
        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
        --
        l_table_redefinition_hist_rec.table_size_mbs_after := ROUND(l_bytes_after / POWER(2,20), 3);
        ...
        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
        --
        l_table_redefinition_hist_rec.all_lobs_size_mbs_after := ROUND(l_sum_bytes_after / POWER(2,20), 3);
        --
        l_statement := 
        q'[SELECT /*+ iod_space.table_redefinition */ ]'||CHR(10)||
        ...
        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
        --
        l_table_redefinition_hist_rec.all_index_size_mbs_after := ROUND(l_sum_bytes_after / POWER(2,20), 3);
        l_table_redefinition_hist_rec.top_index_size_mbs_after := ROUND(l_max_bytes_after / POWER(2,20), 3);
        --
        ...
        output('-- lobs before:'||l_table_redefinition_hist_rec.all_lobs_size_mbs_before||'MBs. after:'||l_table_redefinition_hist_rec.all_lobs_size_mbs_after||'MBs. diff:'||(l_table_redefinition_hist_rec.all_lobs_size_mbs_before - l_table_redefinition_hist_rec.all_lobs_size_mbs_after)||'MBs. savings:'||l_savings_percent||'%.');
        --
        ...
        output('-- table+indexes+lobs before:'||(l_table_redefinition_hist_rec.table_size_mbs_before + l_table_redefinition_hist_rec.all_index_size_mbs_before + l_table_redefinition_hist_rec.all_lobs_size_mbs_before)||'MBs. after:'||(l_table_redefinition_hist_rec.table_size_mbs_after + l_table_redefinition_hist_rec.all_index_size_mbs_after + l_table_redefinition_hist_rec.all_lobs_size_mbs_after)||'MBs. diff:'||((l_table_redefinition_hist_rec.table_size_mbs_before + l_table_redefinition_hist_rec.all_index_size_mbs_before + l_table_redefinition_hist_rec.all_lobs_size_mbs_before) - (l_table_redefinition_hist_rec.table_size_mbs_after + l_table_redefinition_hist_rec.all_index_size_mbs_after + l_table_redefinition_hist_rec.all_lobs_size_mbs_after))||'MBs. savings:'||l_savings_percent||'%.');
        --         
        insert_table_redefinition_hist;
        --
        l_table_count := l_table_count + 1;
        l_size_mbs_before := ROUND(l_size_mbs_before + l_table_redefinition_hist_rec.table_size_mbs_before, 3);
        l_size_mbs_after := ROUND(l_size_mbs_after + l_table_redefinition_hist_rec.table_size_mbs_after, 3);
        l_size_all_mbs_before := ROUND(l_size_all_mbs_before + (l_table_redefinition_hist_rec.table_size_mbs_before + l_table_redefinition_hist_rec.all_index_size_mbs_before + l_table_redefinition_hist_rec.all_lobs_size_mbs_before), 3);
        l_size_all_mbs_after := ROUND(l_size_all_mbs_after + (l_table_redefinition_hist_rec.table_size_mbs_after + l_table_redefinition_hist_rec.all_index_size_mbs_after + l_table_redefinition_hist_rec.all_lobs_size_mbs_after), 3);
