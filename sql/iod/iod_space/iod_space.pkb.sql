...
2019-03-18 rrighett - added lobs_hist data collection
                    - added exception to handle ORA-20011: Approximate NDV failed: ORA-08103: object no longer exists on gather_table_stats
2019-03-19 rrighett - KPT-29 CLEAR_REDEF process added and exception insert after 3 failures
*/
/* ------------------------------------------------------------------------------------ */  
...
---
 --
  output('begin '||TO_CHAR(SYSDATE, gk_date_format));
  DBMS_APPLICATION_INFO.SET_MODULE(UPPER('&&1.')||'.IOD_SPACE.lobs_hist','LOBS_HIST');
  -- count
  SELECT COUNT(*)
    INTO l_count
    FROM &&1..lobs_hist;
  output('&&1..lobs_hist begin count: '||l_count);
  -- main 
  INSERT /* &&1.iod_space.lobs_hist */
  INTO &&1..lobs_hist (
        pdb_name        ,
        OWNER						,
        TABLE_NAME			,
  ...
  LOOP
    ...
    IF l_high_value <= ADD_MONTHS(TRUNC(SYSDATE, 'MM'), -12) THEN
      output('PARTITION:'||RPAD(SUBSTR(i.partition_name, 1, 30), 32)||'HIGH_VALUE:'||TO_CHAR(l_high_value, gk_date_format)||'  BLOCKS:'||i.blocks);
      output('&&1..IOD_SPACE.lobs_hist: ALTER TABLE &&1..lobss_hist DROP PARTITION '||i.partition_name, p_alert_log => 'Y');
      EXECUTE IMMEDIATE q'[ALTER TABLE &&1..lobs_hist SET INTERVAL (NUMTOYMINTERVAL(1,'MONTH'))]';
      EXECUTE IMMEDIATE 'ALTER TABLE &&1..lobs_hist DROP PARTITION '||i.partition_name;
    END IF;
  END LOOP;
  -- count
  SELECT COUNT(*)
    INTO l_count
    FROM &&1..lobs_hist;
  output('&&1..lobs_hist end count: '||l_count);
---
---
  DBMS_APPLICATION_INFO.SET_MODULE(NULL,NULL);
  output('end '||TO_CHAR(SYSDATE, gk_date_format));
...
IS
  ...
  --
  -- CLEAR REDEF IF AN EXEPTION DURING EXECUTION IS TRIGGERED
  PROCEDURE CLEAR_REDEF 
          IS PRAGMA AUTONOMOUS_TRANSACTION;
          v_mview varchar2(100);
        BEGIN
            output('-- &&1..IOD_SPACE.table_redefinition Clear Redef inflight Exception', p_alert_log => 'Y');
            DBMS_APPLICATION_INFO.SET_ACTION('CLEAR REDEF->'||l_table_redefinition_hist_rec.table_name||'('||l_table_redefinition_hist_rec.con_id||')');

            output('pdb:'||l_table_redefinition_hist_rec.pdb_name||'('||l_table_redefinition_hist_rec.con_id||') Mlog owner:'||l_table_redefinition_hist_rec.owner||' table:'||l_table_redefinition_hist_rec.table_name, p_alert_log => 'Y');
            l_statement :=  'DROP MATERIALIZED VIEW LOG ON '||l_table_redefinition_hist_rec.owner||'.'||l_table_redefinition_hist_rec.table_name;
            l_cursor_id := DBMS_SQL.OPEN_CURSOR;
            ...
            INTO   v_mview
            FROM   cdb_mviews
            WHERE  owner = l_table_redefinition_hist_rec.owner
            AND    mview_name like 'REDEF%'
            AND    con_id = l_table_redefinition_hist_rec.con_id;

            output('pdb:'||l_table_redefinition_hist_rec.pdb_name||'('||l_table_redefinition_hist_rec.con_id||') Mview owner:'||l_table_redefinition_hist_rec.owner||' mview:'||v_mview, p_alert_log => 'Y');
            l_statement :=  'DROP MATERIALIZED VIEW '||l_table_redefinition_hist_rec.owner||'.'||v_mview;
            l_cursor_id := DBMS_SQL.OPEN_CURSOR;
            DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => l_table_redefinition_hist_rec.pdb_name);
            l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
            DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);

            output('pdb:'||l_table_redefinition_hist_rec.pdb_name||'('||l_table_redefinition_hist_rec.con_id||') Table owner:'||l_table_redefinition_hist_rec.owner||' table:'||v_mview, p_alert_log => 'Y');
            l_statement :=  'DROP TABLE '||l_table_redefinition_hist_rec.owner||'.'||v_mview;
            l_cursor_id := DBMS_SQL.OPEN_CURSOR;
            DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => l_table_redefinition_hist_rec.pdb_name);
            l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
            DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
  END;
--
-- CLEAR REDEF BEFORE EXECUTING REDEF PROCESS
--
  PROCEDURE BEF_CLEAR_REDEF
          IS PRAGMA AUTONOMOUS_TRANSACTION;
          v_mview varchar2(100);
          pdb_name varchar2(128);
        BEGIN
                 -- clear inconsistent Mlog
            output('-- &&1..IOD_SPACE.table_redefinition DBMS_REDEFINITION.REDEF_TABLE Clear inconsistent Mlog', p_alert_log => 'Y');
            FOR i in  (select log_owner owner, master table_name, con_id from CDB_MVIEW_LOGS l
                       where EXISTS
                              (select 1
                          from &&1..table_redefinition_hist h
                         where h.error_message like 'ORA-23543%'
                            and h.con_id=l.con_id
                            and h.owner=l.log_owner
                            and h.table_name=l.master
                        )
                        order by con_id) LOOP
                        ...
                        DBMS_APPLICATION_INFO.SET_ACTION('CLEAR REDEF->'||I.table_name||'('||i.con_id||')');
                        output('pdb:'||pdb_name||'('||i.con_id||') CDB_MVIEW_LOGS owner:'||i.owner||' table:'||i.table_name, p_alert_log => 'Y');
                        l_statement :=  'DROP MATERIALIZED VIEW LOG ON '||i.owner||'.'||i.table_name;
                        l_cursor_id := DBMS_SQL.OPEN_CURSOR;
                        DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => pdb_name);
                        l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
                        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
            ...
            --Clear Mview and Table Redef from failed executions
            output('-- &&1..IOD_SPACE.table_redefinition DBMS_REDEFINITION.REDEF_TABLE Clear failed executions CDB_REDEFINITION_STATUS', p_alert_log => 'Y');
            FOR i in  (SELECT INTERIM_OBJECT_OWNER owner, INTERIM_OBJECT_NAME mview_name, con_id, BASE_TABLE_OWNER, BASE_TABLE_NAME
                        FROM CDB_REDEFINITION_STATUS WHERE STATUS NOT IN ('Success','In Progress') 
                         AND INTERIM_OBJECT_NAME IS NOT NULL
                         ORDER BY CON_ID)
                       LOOP
                       ...
                       SELECT NAME
                        ...
                        WHERE con_id=i.con_id;
                         ...
                         output('pdb:'||pdb_name||'('||i.con_id||') cdb_mviews owner:'||i.owner||' mview/table:'||i.mview_name, p_alert_log => 'Y');
                        l_statement :=  'DROP MATERIALIZED VIEW '||i.owner||'.'||i.mview_name;
                        l_cursor_id := DBMS_SQL.OPEN_CURSOR;
                        DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => pdb_name);
                        l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
                        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);

                        l_statement :=  'DROP TABLE '||i.owner||'.'||i.mview_name;
                        l_cursor_id := DBMS_SQL.OPEN_CURSOR;
                        DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => pdb_name);
                        l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
                        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
                        
                        l_statement :=  'DROP MATERIALIZED VIEW LOG ON '||i.BASE_TABLE_OWNER||'.'||i.BASE_TABLE_NAME;
                        l_cursor_id := DBMS_SQL.OPEN_CURSOR;
                        DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => pdb_name);
                        l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
                        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
            ...
            -- Clear lost redef objects
            output('-- &&1..IOD_SPACE.table_redefinition DBMS_REDEFINITION.REDEF_TABLE cdb_mviews clear lost objects', p_alert_log => 'Y');
            FOR i IN (SELECT c.name pdb_name,
             ...
             WHERE mv.mview_name LIKE 'REDEF$\_T'||CHR(37) ESCAPE '\'
               ...
               AND NOT EXISTS
                  (SELECT 1 FROM CDB_REDEFINITION_STATUS S
                      WHERE S.STATUS ='In Progress'
                      AND   S.INTERIM_OBJECT_NAME=mv.mview_name
                      AND   S.INTERIM_OBJECT_OWNER=mv.owner
                      AND   S.CON_ID=mv.con_id)
             ORDER BY
                   c.name,
                   mv.owner,
                   mv.mview_name)
                ...
                output('-- &&1..IOD_SPACE.table_redefinition DBMS_REDEFINITION.REDEF_TABLE cdb_tables clear lost objects', p_alert_log => 'Y');
                FOR i IN (SELECT c.name pdb_name,
                                 c.con_id,
                                 tb.owner,
                                 tb.table_name
                           ...
                           WHERE tb.table_name LIKE 'REDEF$\_T'||CHR(37) ESCAPE '\'
                             ...
                             AND c.open_mode = 'READ WRITE'
                                   AND NOT EXISTS
                                              (SELECT 1 FROM CDB_REDEFINITION_STATUS S
                                              WHERE S.STATUS ='In Progress'
                                              AND   S.INTERIM_OBJECT_NAME=tb.table_name
                                              AND   S.INTERIM_OBJECT_OWNER=tb.owner
                                              AND   S.CON_ID=tb.con_id)
                           ORDER BY
                                 c.name,
                                 tb.owner,
                                 tb.table_name)
                LOOP
                  ...
                  output('pdb:'||i.pdb_name||'('||i.con_id||') cdb_tables owner:'||i.owner||' table_name:'||i.table_name, p_alert_log => 'Y');
                        ...
                        l_statement :=  'DROP TABLE '||i.owner||'.'||i.table_name;
                        l_cursor_id := DBMS_SQL.OPEN_CURSOR;
                        DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => i.pdb_name);
                        l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
                        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
...
BEGIN
      ...
      IF (l_savings_percent > l_min_savings_perc OR l_table_redefinition_hist_rec.top_index_size_mbs_before > l_table_redefinition_hist_rec.table_size_mbs_before) AND l_report_only = 'N' THEN
        ...
        EXCEPTION
          WHEN OTHERS THEN
            ...
            output('--- Remediation steps automated clear redef procedure called', p_alert_log => 'Y');
              CLEAR_REDEF;
            /* Removed, clear_redef process automated the clean 
            ...
            */
            ...
            DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
            --
            --RAISE; -- Removed, clear_redef process automated the clean 
        END;
        DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
        --
        -- 
  ...
  output('-- ');
  --
  -- add to exception list if table has more than 3 errors on the last 7 days
  --
    FOR b in (SELECT pdb_name, owner, table_name, count(*) num_errors
              FROM &&1..table_redefinition_hist th
              WHERE error_message is not null
              AND   DDL_BEGIN_TIME >= SYSDATE-7
              AND   NOT EXISTS (SELECT 1 FROM &&1..exceptions_black_list bl
                                WHERE th.owner=bl.owner
                                AND   th.table_name=bl.table_name
                                AND   th.pdb_name=bl.pdb_name )
              GROUP BY pdb_name, owner, table_name) LOOP

          IF b.num_errors >= 3 THEN
             INSERT INTO &&1..exceptions_black_list
             VALUES ('TABLE_REDEFINITION',b.pdb_name, b.owner, b.table_name,'MORE THAN 3 FAILURES KPT-29', SYSDATE);
             COMMIT;
          END IF;
    END LOOP;    
  --
  ...
  l_count := 0;
 /* REMOVED FROM THE VALIDATION PROCESS, CLEAR_REDEF AND BEF_CLEAR_REDEF SHOULD DEAL WITH MVIEW_LOGS
  -- THIS CHECK WAS ALSO REPORTING MLOGS THAT ARE USED BY NON-REDEF PROCESSES.
...
BEGIN
  ...
  FOR i IN (WITH 
            ...
            SELECT DISTINCT -- IOD-16341
                   c.name pdb_name, t.con_id, t.owner, t.table_name,
             ...
             WHERE 1 = 1
               ...
               AND c.con_id = t.con_id
               AND c.open_mode = 'READ WRITE'
               AND c.name NOT LIKE '%CASPER%' -- IOD-16341
             ORDER BY
  ...
  LOOP
    ...
    output(i.pdb_name||' '||l_statement);
    --
    DBMS_SQL.PARSE(c => l_cursor_id, statement => l_statement, language_flag => DBMS_SQL.NATIVE, container => i.pdb_name);
    --
    DECLARE
      l_subscript_beyond_count EXCEPTION; -- IOD-16341
      PRAGMA EXCEPTION_INIT(l_subscript_beyond_count, -06533); -- ORA-06533: Subscript beyond count
      --
      l_unable_to_analyze_table EXCEPTION; -- IOD-24
      PRAGMA EXCEPTION_INIT(l_unable_to_analyze_table, -20000); -- ORA-20000: Unable to analyze TABLE "TANDEN_ENG_USER"."SCHEMA_VERSION", insufficient privileges or does not exist
      --
      l_obj_no_longer_exists EXCEPTION; -- IOD-16525
      PRAGMA EXCEPTION_INIT(l_subscript_beyond_count, -20011); -- ORA-20011: Approximate NDV failed: ORA-08103: object no longer exists
    BEGIN    
      l_rows := DBMS_SQL.EXECUTE(c => l_cursor_id);
    EXCEPTION
      WHEN l_subscript_beyond_count OR l_unable_to_analyze_table OR l_obj_no_longer_exists THEN
        output('*** skip: '||i.pdb_name||' '||i.owner||'.'||i.table_name||' '||SQLERRM);
    END;
    --
    DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
  END LOOP;
