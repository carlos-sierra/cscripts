...
/* $Header: iod_sess.pkb.sql 2018-06-02T00:56:18 carlos.sierra $ */
/* ------------------------------------------------------------------------------------ */  
...
--  
  ...
  IS
    ...
    --
    SELECT /*+ ORDERED */
           CASE WHEN l.ctime >= c_lock_secs_thres AND s.last_call_et >= c_lock_secs_thres THEN 'Y' ELSE 'N' END death_row,
           s.sid,
           s.serial#,
           p.spid,
           s.status,
    ...
    SELECT /*+ ORDERED */
           ...
           CASE WHEN b.ctime >= c_lock_secs_thres AND bs.last_call_et >= c_lock_secs_thres AND w.ctime >= c_lock_secs_thres AND ws.last_call_et >= c_lock_secs_thres THEN 'Y' ELSE 'N' END death_row,
           bs.sid,
           bs.serial#,
           bp.spid,
           bs.status,
...
BEGIN
  ...
  LOOP
    IF (i.type IN ('TM', 'TX') AND l_kill_locked = 'Y') OR (i.type IS NULL AND l_kill_idle = 'Y') THEN
