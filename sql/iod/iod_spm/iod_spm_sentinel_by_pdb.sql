...
SELECT '/tmp/iod_spm_sentinel_&&pdb_name._'||TO_CHAR(SYSDATE, 'YYYYMMDD_HH24MISS') report_file FROM DUAL;
SPO &&report_file..txt;
EXEC c##iod.iod_spm.sentinel(p_report_only => NVL(SUBSTR(UPPER(TRIM('&&report_only.')), 1, 1), 'Y'), p_pdb_name => UPPER(TRIM('&&pdb_name.')));
SPO OFF;
