...
/* $Header: iod_spm.pkb.sql 2018-06-09T02:40:30 carlos.sierra $ */
/* ------------------------------------------------------------------------------------ */  
...
/* ------------------------------------------------------------------------------------ */
  ...
  k_num_rows_appl_4              CONSTANT NUMBER := 5000; -- minimum number of rows on cbo stats for main table to be a candidate
  k_num_rows_noappl              CONSTANT NUMBER := 5000; -- minimum number of rows on cbo stats for main table to be a candidate
  ...
  EXCEPTION
    WHEN self_deadlock THEN
      output('ORA-04024: self-deadlock detected while trying to mutex pin cursor - on display_sql_plan_baseline during '||p_sql_handle||' '||p_plan_name||' '||p_con_name);
      DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
    WHEN sessions_exceeded THEN
  ...
  BEGIN
    ...
    q'{       CASE WHEN p.other_xml IS NOT NULL THEN TO_NUMBER(extractvalue(xmltype(p.other_xml),'/*/info[@type = "plan_hash"]')) END, }'||CHR(10)||
    q'{       CASE WHEN p.other_xml IS NOT NULL THEN TO_NUMBER(extractvalue(xmltype(p.other_xml),'/*/info[@type = "plan_hash_2"]')) END, }'||CHR(10)||
    q'{       CASE WHEN p.other_xml IS NOT NULL THEN TO_NUMBER(extractvalue(xmltype(p.other_xml),'/*/info[@type = "plan_hash_full"]')) END, }'||CHR(10)||
  ...
  EXCEPTION
    WHEN self_deadlock THEN
      ...
      DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
    WHEN sessions_exceeded THEN
      output('ORA-00018: maximum number of sessions exceeded - on sql_plan_baseline_other_xml during '||p_signature||' '||p_plan_name||' scan');
      DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
