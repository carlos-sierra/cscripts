...
/* ------------------------------------------------------------------------------------ */
  ...
  EXCEPTION
    ...
    WHEN sessions_exceeded THEN
      ...
      DBMS_SQL.CLOSE_CURSOR(c => l_cursor_id);
      --RAISE sessions_exceeded; --KPT-61
        ...
        BEGIN -- Output cursor details
          ...
          IF c_rec.mr_snap_id IS NOT NULL THEN
            ...
            IF l_mrs_is_considered THEN
              output('Is this snapshot considered?','YES (i.e. within last '||gl_rec.most_recent_awr_snap_hours||' hours)');      
            ELSE
              output('Is this snapshot considered?','NO (i.e. not within last '||gl_rec.most_recent_awr_snap_hours||' hours)');      
            END IF;        
          ...
          IF gl_rec.display_plan = 'Y' AND (l_spb_exists OR l_spb_demotion_was_accepted OR l_spb_promotion_was_accepted) THEN
            ...
            l_prior := 1;
            l_next := 1;
            IF l_spb_plan IS NOT NULL THEN
              WHILE l_next > 0
              LOOP
