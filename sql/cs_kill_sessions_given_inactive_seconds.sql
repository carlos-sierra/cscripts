...
/
PRO
PRO 2. Enter LAST_CALL_SECS "greater than" to kill sessions older than: (default 3600)
DEF last_call_et = '&2.';
PRO
SET HEA OFF PAGES 0 FEED OFF VER OFF ECHO OFF;
SPO kill_sessions_driver.sql;
SELECT '-- last call: '||e.last_call_et||' seconds'||CHR(10)||
       'ALTER SYSTEM DISCONNECT SESSION '''||e.sid||','||e.serial#||''' IMMEDIATE;' 
  FROM v$session e
 WHERE e.machine LIKE '&&machine.%'
   AND e.type = 'USER'
   AND e.sid <> USERENV('SID')
   AND e.machine <> '&&host_name.'
   AND e.last_call_et > TO_NUMBER(NVL('&last_call_et.', '3600'))
 ORDER BY 
       e.last_call_et
/
SPO OFF;
