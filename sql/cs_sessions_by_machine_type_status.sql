...
COL last_call_secs FOR 999,999,999,990 HEA 'LAST_CALL|SECONDS';
...
COL max_last_call_secs FOR 999,999,999,990 HEA 'MAX LAST_CALL|SECONDS';
--
BREAK ON REPORT;
...
SELECT machine, type, status, COUNT(*) sessions, MIN(last_call_et) AS min_last_call_secs--, MAX(last_call_et) AS max_last_call_secs, ROUND(AVG(last_call_et)) AS avg_last_call_secs
  FROM v_session
 GROUP BY 
       machine, type, status
 ORDER BY
