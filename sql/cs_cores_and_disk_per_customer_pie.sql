----------------------------------------------------------------------------------------
--
-- File name:   cs_cores_and_disk_per_customer_pie.sql 
--
-- Purpose:     DB CPU Cores and Disk Space per Customer
--
...
--              SQL> @cs_cores_and_disk_per_customer_pie.sql
--
...
--
DEF cs_script_name = 'cs_cores_and_disk_per_customer_pie';
--
COL realm FOR A5;
COL rgn FOR A3;
...
CLEAR BREAK COMPUTE;
BREAK ON REPORT;
COMPUTE SUM LABEL 'TOTAL' OF aas_on_cpu used_space_gbs pdbs kiev cdbs ON REPORT;
--
SELECT realm, 
...
/
PRO
PRO 2. Enter RGN or REGION: [{*}|RGN|REGION]
DEF cs_rgn_or_region = '&2.';
UNDEF 2;
...
/
PRO
PRO 3. Enter LOCALE: [{*}|AD1|AD2|AD3|RGN]
DEF cs_locale = '&3.';
UNDEF 3;
...
--
PRO
PRO 6. Enter Metric: [{CPU}|DISK|PDBS]
DEF cs_metric = '&6.';
UNDEF 6;
COL cs_metric NEW_V cs_metric FOR A4 NOPRI TRUNC;
SELECT NVL(UPPER(TRIM('&&cs_metric.')), 'CPU') AS cs_metric FROM DUAL
/
SELECT CASE WHEN '&&cs_metric.' IN ('CPU', 'DISK', 'PDBS') THEN '&&cs_metric.' ELSE 'CPU' END AS cs_metric FROM DUAL
/
...
WHEN 'PDBS' THEN 'Plugable Databases (PDBs)'
END cs_display_metric FROM DUAL
/
--
SELECT '&&cs_file_prefix._&&cs_script_name.' cs_file_name FROM DUAL;
--
DEF report_title = '&&cs_display_metric. per Customer';
...
DEF report_foot_note = 'SQL> @&&cs_script_name..sql "&&cs_realm." "&&cs_rgn_or_region." "&&cs_locale." "&&cs_dbname." "&&cs_kiev_only." "&&cs_metric."';
--
...
SELECT 
  customer_oci_service
, COALESCE(customer_oci_service, 'PDB:'||pdb_name) AS customer_or_pdb
...
, COUNT(DISTINCT jdbc_connect_string) AS pdbs           
, ROUND(100 * COUNT(DISTINCT jdbc_connect_string) / SUM(COUNT(DISTINCT jdbc_connect_string)) OVER (), 3) AS pdbs_p
, SUM(CASE kiev WHEN 'Y' THEN 1 ELSE 0 END) AS kiev
, COUNT(DISTINCT host_name) AS cdbs
  FROM c##iod.resources_per_pdb_v
 WHERE '&&cs_realm.' IN ('*', realm)
...
SELECT /*+ MATERIALIZE NO_MERGE */
       customer_or_pdb slice,
       CASE '&&cs_metric.' 
         WHEN 'CPU' THEN ROUND(aas_on_cpu) 
         WHEN 'DISK' THEN ROUND(used_space_gbs) 
         WHEN 'PDBS' THEN pdbs
       END AS value,
       ' ('||TRIM(TO_CHAR(CASE '&&cs_metric.' 
                            WHEN 'CPU' THEN ROUND(aas_on_cpu_p, 1) 
                            WHEN 'DISK' THEN ROUND(used_space_gbs_p, 1) 
                            WHEN 'PDBS' THEN ROUND(pdbs_p, 1) 
                          END, '990.0'))||'%)' AS percent
  FROM resources_per_pdb
)
SELECT ', ['''||TRIM(TO_CHAR(value, '999,990'))||' '||slice||percent||''','||value||']'
  FROM summary
 ORDER BY
       value DESC
/
...
PRO METRIC       : "&&cs_metric." [{CPU}|DISK|PDBS]
PRO </pre>
