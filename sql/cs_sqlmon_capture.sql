...
BEGIN
  ...
  $THEN
    DBMS_SQLDIAG_INTERNAL.i_create_patch(sql_text => l_sql_text, hint_text => q'[&&cbo_hints. &&cs_additional_cbo_hints.]', name => 'spch_&&cs_sql_id._&&hash.', description => q'[cs_sqlmon_capture.sql /*+ &&cbo_hints. &&cs_additional_cbo_hints. */ &&cs_reference_sanitized.]'); -- 12c
  $ELSE
    l_name := DBMS_SQLDIAG.create_sql_patch(sql_text => l_sql_text, hint_text => q'[&&cbo_hints. &&cs_additional_cbo_hints.]', name => 'spch_&&cs_sql_id._&&hash.', description => q'[cs_sqlmon_capture.sql /*+ &&cbo_hints. &&cs_additional_cbo_hints. */ &&cs_reference_sanitized.]'); -- 19c
  $END
  NULL;
END;
/
