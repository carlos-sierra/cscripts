-- collect_ORA-600_diagnostics.sql (2020/12/09)
-- collects ORA-600 diagnostics. inputs PDB and SQL_ID.
SPO collect_ORA-600_qksanGetTextStr_diagnostics.log
SET HEA ON LIN 500 PAGES 100 TAB OFF FEED OFF ECHO OFF VER OFF TRIMS ON TRIM ON TI OFF TIMI OFF;
ALTER SESSION SET container = CDB$ROOT;
SELECT name pdb_name FROM v$containers WHERE open_mode = 'READ WRITE' ORDER BY 1;
PRO 1. Enter PDB_NAME failing with ORA-600:
DEF pdb_name = '&1.';
UNDEF 1;
PRO 2. Enter SQL_ID failing with ORA-600:
DEF sql_id = '&2.';
UNDEF 2;
...
PRO 1. verify on alert log ORA-600[qksanGetTextStr] is no longer raised
