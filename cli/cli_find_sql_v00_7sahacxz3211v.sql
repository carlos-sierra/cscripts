...
DEF search_string = 'performScanQuery(NOTIFICATION_BOARD,EVENT_BY_SCHEDULED_TIME)%AND (notificationType = :2 ) AND (taskStatus = :3 )';
-- 
DEF low_value = '0';
...
HAVING NVL(SUM(s.executions), 0) BETWEEN &&executions_min. AND &&executions_max.
 ...
 ORDER BY
       1, 2, 3, 4 DESC NULLS LAST, 6 DESC NULLS LAST, 7 DESC NULLS LAST
/
--
SELECT /* &&cs_script_name. */
       s.sql_id,
