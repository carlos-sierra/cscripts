...
DEF search_string = '/* performSnapshot%Query(';
-- 
DEF low_value = '0';
...
DEF ms_per_row_min = '&&low_value.';
DEF ms_per_row_max = '&&high_value.';
...
SELECT /* &&cs_script_name. */
       ...
       (SELECT version FROM v$instance WHERE ROWNUM = 1) AS db_version,
       'Y' please_stop
  FROM v$sql s
          CROSS APPLY (
 ...
 WHERE '&&please_stop.' IS NULL -- short-circuit if a prior sibling query returned rows
   ...
   AND (s.sql_id, s.con_id) IN (
                            SELECT DISTINCT sql_id, con_id
                              FROM v$sql_plan
                            WHERE operation = 'INDEX'
                              AND options IN ('FAST FULL SCAN', 'FULL SCAN', 'SKIP SCAN')
                              AND object_name LIKE 'KIEV\_S%' ESCAPE '\'
   )
 GROUP BY
       s.sql_id,
...
HAVING NVL(SUM(s.executions), 0) BETWEEN &&executions_min. AND &&executions_max.
   ...
   AND NVL(SUM(s.elapsed_time)/NULLIF(GREATEST(SUM(s.rows_processed), SUM(s.executions)),0)/1e3, 0) BETWEEN &&ms_per_row_min. AND &&ms_per_row_max.
