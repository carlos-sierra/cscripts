DEF cs_top_latency = '40000';
DEF cs_top_load = '20000';
DEF cs_aas_threshold_latency = '0.001';
DEF cs_aas_threshold_load = '0.010';
...
/****************************************************************************************/
delta AS (
SELECT /*+ MATERIALIZE NO_MERGE GATHER_PLAN_STATISTICS MONITOR */ ((86400 * EXTRACT(DAY FROM (SYSTIMESTAMP - MAX(end_interval_time))) + (3600 * EXTRACT(HOUR FROM (systimestamp - MAX(end_interval_time)))) + (60 * EXTRACT(MINUTE FROM (systimestamp - MAX(end_interval_time)))) + EXTRACT(SECOND FROM (systimestamp - MAX(end_interval_time))))) AS seconds FROM dba_hist_snapshot WHERE end_interval_time < SYSTIMESTAMP
),
sqlstats AS (
SELECT /*+ MATERIALIZE NO_MERGE GATHER_PLAN_STATISTICS MONITOR */
       ...
       s.delta_elapsed_time/GREATEST(delta.seconds,1)/1e6 AS et_aas,
       s.delta_cpu_time/GREATEST(delta.seconds,1)/1e6 AS cpu_aas,
       s.delta_user_io_wait_time/GREATEST(delta.seconds,1)/1e6 AS io_aas,
       s.delta_application_wait_time/GREATEST(delta.seconds,1)/1e6 AS appl_aas,
       s.delta_concurrency_time/GREATEST(delta.seconds,1)/1e6 AS conc_aas,
       s.delta_plsql_exec_time/GREATEST(delta.seconds,1)/1e6 AS plsql_aas,
       s.delta_execution_count/GREATEST(delta.seconds,1) AS execs_per_sec,
       s.delta_px_servers_executions/GREATEST(delta.seconds,1) AS px_execs_per_sec,
       s.delta_end_of_fetch_count/GREATEST(delta.seconds,1) AS end_of_fetch_per_sec,
       s.delta_parse_calls/GREATEST(delta.seconds,1) AS parses_per_sec,
       s.delta_invalidations/GREATEST(delta.seconds,1) AS inval_per_sec,
       s.delta_loads/GREATEST(delta.seconds,1) AS loads_per_sec,
       ...
       s.delta_elapsed_time/GREATEST(s.delta_rows_processed,1)/1e3 AS et_ms_per_row,
       s.delta_cpu_time/GREATEST(s.delta_rows_processed,1)/1e3 AS cpu_ms_per_row,
       s.delta_user_io_wait_time/GREATEST(s.delta_rows_processed,1)/1e3 AS io_ms_per_row,
       s.delta_buffer_gets/GREATEST(s.delta_rows_processed,1) AS gets_per_row,
       s.delta_disk_reads/GREATEST(s.delta_rows_processed,1) AS reads_per_row,
       s.sql_id,
       s.sql_text,
       s.sql_fulltext,
       s.plan_hash_value,
       s.last_active_child_address
  FROM v$sqlstats s, delta
...
SELECT /*+ MATERIALIZE NO_MERGE */
    --    s.type,
       ...
       s.con_id,
       c.name pdb_name
    --    ROW_NUMBER() OVER (PARTITION BY s.type ORDER BY s.et_ms_per_exec DESC) AS latency_rn,
    --    ROW_NUMBER() OVER (PARTITION BY s.type ORDER BY s.et_aas DESC) AS load_rn
  FROM sqlstats s,
       v$containers c
 WHERE 1 = 1 --s.type IN ('TP', 'RO', 'BG', 'UN')
...
SELECT /*+ GATHER_PLAN_STATISTICS MONITOR */ COUNT(*) FROM results
/
