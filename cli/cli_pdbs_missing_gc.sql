...
/*****************************************************************************************************************/
pdbs_recently_active AS ( -- pdbs with tables having inserts or deletes within the last 6 hours
SELECT /*+ MATERIALIZE NO_MERGE */
       pdb_name,
       COUNT(*) AS active_tables,
       SUM(inserted_in_window) AS tables_inserted_in_window,
       SUM(deleted_in_window) AS tables_deleted_in_window,
       ROUND(100 * SUM(deleted_in_window) / NULLIF(SUM(inserted_in_window), 0)) AS perc_tables_with_gc,
       SUM(CASE WHEN inserts > 0 THEN 1 ELSE 0 END) AS tables_with_inserts,
       SUM(CASE WHEN deletes > 0 THEN 1 ELSE 0 END) AS tables_with_deletes
  FROM active_tables_within_window
 GROUP BY
       pdb_name
),
pdbs_missing_gc AS ( -- pdbs with more than one table having inserts within the last 6 hours and no tables having deletes within the last 6 hours
SELECT /*+ MATERIALIZE NO_MERGE */
       pdb_name
  FROM pdbs_recently_active
 WHERE perc_tables_with_gc < &&perc_tables_gc.
)
SELECT COUNT(*) AS pdbs_missing_gc,
       SUBSTR(LISTAGG(pdb_name, ', ') WITHIN GROUP (ORDER BY 1), 1, 200) AS list_of_pdbs
  FROM pdbs_missing_gc
/
