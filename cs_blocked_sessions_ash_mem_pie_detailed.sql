----------------------------------------------------------------------------------------
--
-- File name:   cs_blocked_sessions_ash_mem_pie_detailed.sql
--
-- Purpose:     Top Session Blockers as per ASH from Memory (detailed pie chart)
--
...
--              SQL> @cs_blocked_sessions_ash_mem_pie_detailed.sql
--
...
--
DEF cs_script_name = 'cs_blocked_sessions_ash_mem_pie_detailed';
...
--
DEF report_title = 'Root Blocker Machine/Status/SQL_ID and its contribution to Blocked Sessions between &&cs_sample_time_from. and &&cs_sample_time_to. UTC';
...
summary AS (
SELECT /*+ MATERIALIZE NO_MERGE */
       blocker_machine||' '||blocker_status||' '||blocker_sql_id slice,
       ' ('||TRIM(TO_CHAR(100 * SUM(blocked) / SUM(SUM(blocked)) OVER (), '990.0'))||'%)' percent,
       SUM(blocked) value
  FROM detail
 GROUP BY
       blocker_machine||' '||blocker_status||' '||blocker_sql_id
)
