...
t AS (SELECT /*+ OPT_PARAM('_px_cdb_view_enabled' 'FALSE') MATERIALIZE NO_MERGE */ con_id, dbid, sql_id, REPLACE(REPLACE(DBMS_LOB.substr(sql_text, 1000), CHR(10), CHR(32)), CHR(9), CHR(32)) AS sql_text FROM dba_hist_sqltext WHERE ('&&sql_text_piece.' IS NULL OR UPPER(DBMS_LOB.substr(sql_text, 1000)) LIKE UPPER('%&&sql_text_piece.%'))),
sqlstat1 AS (
      SELECT /*+ MATERIALIZE NO_MERGE */
            ...
            MAX(h.sharable_mem) AS sharable_mem,
            MAX(h.version_count) AS version_count,
            SUM(h.fetches_delta) AS fetches_delta,
            SUM(h.executions_delta) AS executions_delta,
            SUM(h.parse_calls_delta) AS parse_calls_delta,
            SUM(h.disk_reads_delta) AS disk_reads_delta,
            SUM(h.buffer_gets_delta) AS buffer_gets_delta,
            SUM(h.rows_processed_delta) AS rows_processed_delta,
            SUM(h.cpu_time_delta) AS cpu_time_delta,
            SUM(h.elapsed_time_delta) AS elapsed_time_delta,
            SUM(h.iowait_delta) AS iowait_delta,
            SUM(h.clwait_delta) AS clwait_delta,
            SUM(h.apwait_delta) AS apwait_delta,
            SUM(h.ccwait_delta) AS ccwait_delta,
            SUM(h.plsexec_time_delta) AS plsexec_time_delta,
            SUM(h.javexec_time_delta) AS javexec_time_delta
...
sqlstat2 AS (
      SELECT /*+ OPT_PARAM('_px_cdb_view_enabled' 'FALSE') MATERIALIZE NO_MERGE */
            ...
            h.parse_calls_delta / &&cs_begin_end_seconds. AS parse_calls_ps,
            ...
            h.clwait_delta / POWER(10, 6) / &&cs_begin_end_seconds. AS cl_aas,
            h.plsexec_time_delta / POWER(10, 6) / &&cs_begin_end_seconds. AS pl_aas,
            h.javexec_time_delta / POWER(10, 6) / &&cs_begin_end_seconds. AS ja_aas,
            t.sql_text,
            application_category(t.sql_text, 'UN') AS sql_type,
            c.name AS pdb_name
      FROM  sqlstat1 h, t,
            v$containers c
      --  CROSS APPLY (
