/* -------------------------------------------------------------------------------------- */
--
-- File name:   /tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT.sql
--
...
--              SQL> @/tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT.sql
--
/* -------------------------------------------------------------------------------------- */
...
--
SPO /tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT_&&report_time..txt;
PRO PRO
PRO PRO /tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT_&&report_time..txt
PRO PRO
--
...
--
PRO
PRO /* populateBucketGCWorkspace */ INSERT INTO KIEVGCUSER.KievGCTempTable (TxnId) SELECT DISTINCT(KievT
PRO [17738517391815965579][962sk02ct05pk][1172229985][2723483158][][][NORMAL][KIEVGCUSER]
DECLARE
...
(
SELECT BI.KievTxnID
FROM MAPPING_UPDATES_rgn BI
WHERE BI.KievTxnId <= :1
AND (BI.KievTxnId <            (SELECT MAX(BI2.KievTxnID) FROM MAPPING_UPDATES_rgn BI2 WHERE BI2.mappingKind = BI.mappingKind AND BI2.entityId = BI.entityId AND BI2.updateTimestamp = BI.updateTimestamp AND BI2.KievTxnId <= :2 )
OR BI.KievLive = 'N')
...
q'[INDEX(@"SEL$3" "BI2"@"SEL$3" ("MAPPING_UPDATES_RGN"."MAPPINGKIND" "MAPPING_UPDATES_RGN"."ENTITYID" "MAPPING_UPDATES_RGN"."UPDATETIMESTAMP" "MAPPING_UPDATES_RGN"."KIEVTXNID"))]',
q'[END_OUTLINE_DATA]'
);
-- transformations
...
l_plan_name := 'exp_962sk02ct05pk_210910204502';
l_description := '['||l_target_signature||'][1172229985][2723483158][17738517391815965579][962sk02ct05pk][][][BOM][KIEV02RG][SERVICEGATEWAYBOM][EXP][NORMAL][DBPERF-7353]';
...
IF NVL(i.description, 'NULL') NOT LIKE '%[210910204502]%' THEN
IF LENGTH(i.description) > 470 THEN
l_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => 'DESCRIPTION', attribute_value => '[EXP][210910204502]');
ELSE
l_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => 'DESCRIPTION', attribute_value => i.description||' [EXP][210910204502]');
END IF;
o('SPBL update: [EXP][210910204502]');
END IF;
END LOOP;
END;
/
...
SET SERVEROUT OFF;
--
-- /tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT.sql
--
HOS ls -l /tmp/DBPERF-7353_2021-09-10T20.44.49Z_OC1_BOM_RGN_KIEV02RG_SERVICEGATEWAYBOM_cs_sprf_export_IMPLEMENT*
