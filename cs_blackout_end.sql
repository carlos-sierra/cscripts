...
--
-- Purpose:     Blackout End - for IOD/DBPERF Jobs
--
...
EXEC &&cs_tools_schema..IOD_LOG.configure(logName => 'cs_blackout_end.sql', logToOutput => 'Y', logToAlertLog => 'Y'); 
PRO
PRO Blackout Status
PRO ~~~~~~~~~~~~~~~
...
PRO
PRO Ending Blackout and Open Automatic Maintenance Windows (please wait)
PRO ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
DECLARE
...
BEGIN    
  ...
  --
  IF SYSDATE BETWEEN l_rec.begin_time AND l_rec.end_time THEN
    ...
    IF l_rec.locked = 'Y' THEN
      &&cs_tools_schema..IOD_LOG.info(msg => 'blackout locked!');
    ELSE
      &&cs_tools_schema..IOD_LOG.info(msg => 'Ending Blackout');
      &&cs_tools_schema..iod_admin.set_blackout(p_minutes => 0, p_part_maint_blackout => 0);
      &&cs_tools_schema..IOD_LOG.info(msg => 'Open Automatic Maintenance Windows');
      &&cs_tools_schema..iod_amw.enable_pdb_auto_maint_windows;
    END IF;
  ELSE
