/* -------------------------------------------------------------------------------------- */
--
-- File name:   /tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK.sql
--
...
--              SQL> @/tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK.sql
--
/* --------------------------------------------------------------------------------------- */
...
--
SPO /tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK_&&report_time..txt;
PRO PRO
PRO PRO /tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK_&&report_time..txt
PRO PRO
...
o('~~~~~~~~~~~~~~~~~~~~~~~~~~');
FOR i IN (SELECT name FROM dba_sql_profiles WHERE name LIKE 'exp_%_210721224646' AND category = 'DEFAULT' AND status = 'ENABLED' AND description LIKE '%][EXP][%')
LOOP
o('SPRF drop: '||i.name);
...
o('~~~~~~~~~~~~~~~~~~~~~~~~~');
FOR i IN (SELECT name FROM dba_sql_profiles WHERE name NOT LIKE 'exp_%_210721224646' AND category = 'BACKUP' AND status = 'ENABLED' AND NVL(description, 'NULL') NOT LIKE '%][EXP][%')
LOOP
...
o('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
FOR i IN (SELECT p.sql_handle, p.plan_name, p.description FROM dba_sql_plan_baselines p WHERE p.enabled = 'NO' AND p.accepted = 'YES' AND p.description  LIKE '%[210721224646]%')
LOOP
...
SET SERVEROUT OFF;
--
-- /tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK.sql
--
HOS ls -l /tmp/DBPERF-7399_2021-07-21T22.46.19Z_OC1_ICN_RGN_KIEV02RG_NETWORK_CONTROL_PLANE_cs_sprf_export_ROLLBACK*
