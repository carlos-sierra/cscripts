/* -------------------------------------------------------------------------------------- */
--
-- File name:   /tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT.sql
--
...
--              SQL> @/tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT.sql
--
/* -------------------------------------------------------------------------------------- */
...
--
SPO /tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT_&&report_time..txt;
PRO PRO
PRO PRO /tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT_&&report_time..txt
PRO PRO
--
...
--
PRO
PRO /* performScanQuery(workflowInstances,I_GC_INDEX) [1008] */ SELECT  *  FROM workflowInstances WHERE
PRO [6985770330825096306][ggvcvmfw6m6h4][472260233][2568182846][workflowInstances][1008][BUCKET]
DECLARE
...
-- sql_text
l_sql_text_clob := q'[
/* performScanQuery(workflowInstances,I_GC_INDEX) [1008] */ SELECT  *
...
AND KievLive = 'Y'
AND (1 = 1)
ORDER BY completionTimeMillis ASC, id ASC
...
q'[INDEX(@"SEL$5DA710D3" "WORKFLOWINSTANCES"@"SEL$1" ("WORKFLOWINSTANCES"."COMPLETIONTIMEMILLIS" "WORKFLOWINSTANCES"."ID" "WORKFLOWINSTANCES"."KIEVTXNID"))]',
q'[NO_ACCESS(@"SEL$5DA710D3" "VW_NSO_1"@"SEL$5DA710D3")]',
q'[LEADING(@"SEL$5DA710D3" "WORKFLOWINSTANCES"@"SEL$1" "VW_NSO_1"@"SEL$5DA710D3")]',
q'[USE_NL(@"SEL$5DA710D3" "VW_NSO_1"@"SEL$5DA710D3")]',
q'[INDEX_DESC(@"SEL$C6423BE4" "WORKFLOWINSTANCES"@"SEL$2" ("WORKFLOWINSTANCES"."ID" "WORKFLOWINSTANCES"."KIEVTXNID"))]',
q'[END_OUTLINE_DATA]'
);
-- transformations
IF '1008' IS NOT NULL THEN -- KIEV Statement Caching specific
EXECUTE IMMEDIATE 'SELECT TO_CHAR(bucketid) AS bucket_id FROM &&kievbuckets_owner..kievbuckets WHERE UPPER(name) = UPPER(''workflowInstances'')' INTO l_target_bucket_id;
END IF;
IF l_target_bucket_id IS NOT NULL THEN
l_sql_text_clob := REPLACE(l_sql_text_clob, '[1008]', '['||l_target_bucket_id||']');
l_target_signature := DBMS_SQLTUNE.sqltext_to_signature (sql_text => l_sql_text_clob);
ELSE
l_target_signature := 6985770330825096306;
END IF;
o('['||l_target_signature||']['||l_target_bucket_id||']');
l_plan_name := 'exp_ggvcvmfw6m6h4_210514223926';
l_description := '['||l_target_signature||'][472260233][2568182846][6985770330825096306][ggvcvmfw6m6h4][workflowInstances][1008][SEA][IOD99RG][OVERLAY_ORM_WFAAS][EXP][BUCKET][DBPERF-7140]';
...
IF NVL(i.description, 'NULL') NOT LIKE '%[210514223926]%' THEN
IF LENGTH(i.description) > 470 THEN
l_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => 'DESCRIPTION', attribute_value => '[EXP][210514223926]');
ELSE
l_plans := DBMS_SPM.alter_sql_plan_baseline(sql_handle => i.sql_handle, plan_name => i.plan_name, attribute_name => 'DESCRIPTION', attribute_value => i.description||' [EXP][210514223926]');
END IF;
o('SPBL update: [EXP][210514223926]');
END IF;
END LOOP;
END;
/
...
SET SERVEROUT OFF;
--
-- /tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT.sql
--
HOS ls -l /tmp/DBPERF-7140_2021-05-14T22.39.26Z_SEA_RGN_IOD99RG_OVERLAY_ORM_WFAAS_cs_sprf_export_IMPLEMENT*
